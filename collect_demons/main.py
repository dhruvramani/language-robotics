...
if __name__ == "__main__":
    ...
    if demon_config.collect_by == 'teleop':
        ...
        # _, episode_id, task_id = file_storage.get_random_trajectory()
        ...
        import random
        traj_db = deg.config.traj_db
        env = deg.get_env()
        for traj in traj_db.objects.all():
            task = deg.env_obj._string_to_task(traj.task_id)        
            env = deg.env_obj.get_task(task)
            obs = env.reset()
            instruction = random.choice(obs[0])
            save_success = file_storage.save_instruct_traj(traj.episode_id, instruction)
            print("{}-{}, instruct: '{}', save : {}".format(traj.task_id, traj.episode_id, instruction, save_success))